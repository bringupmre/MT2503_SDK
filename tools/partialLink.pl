...
sub partialLink
{
    ...
    while ($pass < $ptrHahOpt->{passLimit})
    {
        ...
        $Current_time = time;
        system("echo T_S,partialLinkLib,P,$Current_time >> $DirCur/PlinkTime.log");
        &partialLinkLib($ptrHahLib, \%rule, \%hint, $ptrHahError->{ptrHahLibErr});
        $Current_time = time;
        system("echo T_E,partialLinkLib,P,$Current_time >> $DirCur/PlinkTime.log");

        # Link all by using command from a file specified in option.
        if ($ENV{'MTK_INTERNAL'} eq 'TRUE')
        {	    
        	    my $LicTmp = $ENV{'ARMLMD_LICENSE_FILE'};
	            $ENV{'ARMLMD_LICENSE_FILE'} = '8224@mtklic07';
	            $ret = &finalLink("--mangled", $fileLinkOption, $ptrHahError);
              $ENV{'ARMLMD_LICENSE_FILE'} = $LicTmp;
        }
        else
        {
        	$ret = &finalLink("--mangled", $fileLinkOption, $ptrHahError);
...
{
	  $Current_time = time;
    system("echo T_S,Plink,P,$Current_time > $DirCur/PlinkTime.log");
    my ($argc, $argv) = @_;
    # Set options.
    my %opt = (
        "reGenRule" => 1,
    ...
    $plink_end = time;
    $Current_time = time;
    system("echo T_E,Plink,P,$Current_time >> $DirCur/PlinkTime.log");
